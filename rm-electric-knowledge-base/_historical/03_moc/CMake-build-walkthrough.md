...
> **废弃说明：** 2026-07-24 的 CMake 重构（`132fc1c`）将板级重复逻辑提取到 `cmake/board_common.cmake`，
> `generate_headers.cmake` 替换为 `configure_file` + `.h.in` 模板，
> 板级 CMakeLists.txt 从 ~130 行精简到 ~5 行。
> 此版本仅作历史参考，详见 `03_moc/CMake-build-walkthrough.md`（v2）。
