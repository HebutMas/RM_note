# generate_headers.cmake — CMake 变量到 C 宏的翻译器（已废弃）

> **废弃说明：** 2026-07-24 的 CMake 重构（`132fc1c`）引入了 `configure_file` + `.h.in` 模板方案，
> 替代了本文件中的 `file(WRITE)` + 自定义 `_gen_cmakedefine` 函数。
> 本文件仅作历史参考，现有代码统一使用 [[02_code_twin/apps/robot_def-h-in]] 和
> [[02_code_twin/apps/module_config-h-in]]。
