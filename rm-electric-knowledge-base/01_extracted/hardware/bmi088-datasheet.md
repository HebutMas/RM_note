...
> 出处：BMI088 Data sheet (BST-BMI088-DS000-19, Rev 1.9, Jan 2024)，[[00_raw/hardware/BMI088_Datasheet.pdf|PDF 原件]]
...
> 这是 BMI088 最经典的坑。出处：数据手册第 3 章 "Quick Start Guide – Device Initialization"（第 13 页）和第 6.1 章 "SPI"（第 45 页）。
...
> 出处：数据手册第 24 页 "Register Map" 和第 48 页 I2C 地址
...
> 出处：数据手册第 28 页寄存器表，源码 `module_bmi088.c → Module_BMI088_get_accel()`
...
**灵敏度**（出处：数据手册第 8 页 + 源码 `BMI088_reg.h`）：
...
> 出处：数据手册第 37 页寄存器表，源码 `module_bmi088.c → Module_BMI088_get_gyro()`
...
> 出处：数据手册第 30 页寄存器表，源码 `module_bmi088.c → Module_BMI088_get_temp()`
...
> 出处：数据手册第 13 页 "Quick Start Guide"，源码 `module_bmi088.c` 初始化表
...
> 出处：数据手册第 7-8 页 Electrical Specification + Bosch 官方页面
