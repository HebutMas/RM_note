...
`string(TOUPPER 输入 输出变量)` 把字符串转大写，结果存入输出变量。后面用 `ROBOT_${ROBOT_UPPER}` 拼接出 `ROBOT_INFANTRY3`。
...
直接写一个头文件到磁盘。`${ROBOT_UPPER}` 在配置阶段被替换为 `INFANTRY3`。

### file(GLOB_RECURSE) - 递归扫描文件

递归扫描 `apps/infantry3/single_board/` 下所有子目录的 `.c` 文件，收集到 `_robot_sources` 变量。
...
板级 CMakeLists.txt
  └─ include(config.cmake)
       ...
       └─ include(infantry3/robot.cmake)  ← 加载差异配置
