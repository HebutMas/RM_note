...
**为什么不用磁力计**：机器人车架上有大量铁质结构和电机，磁场干扰严重，磁力计数据不可靠。因此纯 6 轴融合。
...
> ins模块和bmi088部分强耦合,部分bmi088代码。详见 [[02_code_twin/modules/BMI088/module_bmi088]]。
...
| 索引                                | 物理量   | 对应旋转轴 | 单位     |
| --------------------------------- | ----- | ----- | ------ |
| `euler_angle[0]` / `euler_rad[0]` | roll  | 绕 Y 轴 | 度 / 弧度 |
| `euler_angle[1]` / `euler_rad[1]` | pitch | 绕 X 轴 | 度 / 弧度 |
| `euler_angle[2]` / `euler_rad[2]` | yaw   | 绕 Z 轴 | 度 / 弧度 |
