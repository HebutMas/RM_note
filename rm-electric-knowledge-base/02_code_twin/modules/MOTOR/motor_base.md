...
2.底盘（本车实测）**左侧**两个 M3508 的 `motor_reverse_flag = 1`（`[1,1,0,0]`）。麦轮底盘左右电机安装方向相反，运动学算出的 wheel_speed 是"底盘坐标系下的轮速"，**左侧**电机需要取反才能让轮子实际转对方向。具体场景见底盘控制模块。⚠️ 此值是**本车硬件台账**：换车/换接线必须重测轮位与编码方向（对角换槽只翻 vw、vx/vy 检测不出，见 chassis_func 笔记）。
...
| 底盘典型值 | 全 0（用电机自身编码器） | 本车 `[1,1,0,0]`（左对称，2026-08-26 实测）；旧版曾写 `[0,0,1,1]` 为对侧装法，**不可移植** |
| 云台典型值 | pitch=1（INS 方向与电机相反） | pitch=1（本车实测，遥控"上推=低头"，见 gimbal_func） |
