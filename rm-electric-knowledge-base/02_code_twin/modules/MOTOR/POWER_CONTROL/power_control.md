...
`dji_output_to_torque()` 应该把 PID 算出来的 `controller.output`（整数电流值）反算回输出轴扭矩：

```c
// power_control.c — dji_output_to_torque  (有 bug)
...
**Bug：函数名为 `dji_output_to_torque`，但实际只返回了电流值(A)，漏乘了 `Kt_gear`。** 算了 `Kt_gear` 却没用，`IntegerToCurrent` 返回的是电流(A)，应该再乘 `Kt_gear` 才能得到输出轴扭矩(Nm)：

```c
// 应该是:
return IntegerToCurrent(...) * Kt_gear;
// 实际写的是:
return IntegerToCurrent(...);   // 只返回了电流，没乘 Kt_gear
```

这会导致功率估算中 `τ·Ω` 和 `k2·τ²` 项算出来的值偏小（用的是电流值而非扭矩值），功率限制的触发时机和分配比例都会受影响。

`torque_constant` 注释写的是"减速前扭矩常数 (Nm/A)"——电机本身的绕组常数，不含减速比。所以从电流算输出轴扭矩要乘 `Kt × gear_ratio`：先乘 Kt 得电机轴扭矩，再乘减速比得输出轴扭矩。但因为上面的 bug，实际代码没乘。
