...
额外强制转 lost：相机断帧 dt>0.1s；`target.diverged()`（半径出 [0.05,0.5]m）；近窗口 NIS 卡方失败率 ≥40%（`tracker.cpp:83-90`）。**前哨站特判**：转速恒定 ~2.512 rad/s，收敛后强制钳位（`target.cpp:132-133`）。
...
本母文档建立了全局框架，以下专题可通过 `/gen-guide` 模式二逐个深化：

- [[SP_Vision_装甲板识别与解算详解]] — 传统灯条几何/去重、YOLO 三模型解码细节、PnP+yaw 优化重投影推导
- [[SP_Vision_整车估计EKF详解]] — 11 维状态雅可比逐项推导、自适应 R、NIS/NEES 卡方检验、状态机
- [[SP_Vision_轨迹规划器详解]] — 轨迹理论完整推导、TinyMPC/ADMM 求解、五次多项式方案、开火决策
- [[SP_Vision_打符与全向感知详解]] — 大符正弦 RANSAC 拟合、扇叶跳变补偿、多相机架构
- [[SP_Vision_IO与多线程架构详解]] — imu_at 对齐算法、通信协议逐字段、相机多态、线程模型
